        ...
        <el-table :data="list" stripe border style="width: 100%">
            ...
            >
                ...
                <template slot-scope="scope">
                    <el-tag
                        v-if="scope.row.isAccountNonExpired === 0"
                        type="danger"
                        >过期</el-tag
                    >
                    <el-tag
                        v-if="scope.row.isAccountNonExpired === 1"
                        type="success"
                        >正常</el-tag
                    >
                </template>
            ...
            >
                ...
                <template slot-scope="scope">
                    ...
                    >
                    <el-tag
                        v-if="scope.row.isAccountNonLocked === 1"
                        type="success"
                        >正常</el-tag
                    >
                </template>
            ...
            >
                ...
                <template slot-scope="scope">
                    <el-tag
                        v-if="scope.row.isCredentialsNonExpired === 0"
                        type="danger"
                        >过期</el-tag
                    >
                    <el-tag
                        v-if="scope.row.isCredentialsNonExpired === 1"
                        type="success"
                        >正常</el-tag
                    >
                </template>
            ...
            <el-table-column align="center" prop="isEnabled" label="是否可用">
                <!-- (1 可用，0 删除用户) -->
                <template slot-scope="scope">
                    <el-tag v-if="scope.row.isEnabled === 0" type="danger"
                        >已删除</el-tag
                    >
...
export default {
    ...
    methods: {
        ...
        handleRole(id) {
            this.role.userId = id;
            api.getRoleIdsByUserId(id).then((data) => {
                // 角色id 传递给子组件
        ...
        saveUserRole(roleIds) {
            ...
            // 保存用户角色信息
            api.saveUserRole(this.role.userId, roleIds).then(() => {
